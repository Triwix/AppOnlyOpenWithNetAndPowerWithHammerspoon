...
local config = {
    ...
    behavior = {
        ...
        profileEnabled = true, -- Enable lightweight performance profiling output.
        profileReportEvery = 40, -- Emit summary every N profiled calls (0 disables automatic summaries).
        profileTopEntries = 12, -- Number of heaviest labels to print per summary.
        profileResetAfterReport = false, -- Clear accumulated profiler stats after each summary.
...
local function profilingEnabled()
    return config.behavior.profileEnabled == true
end

local function profilerNowMilliseconds()
...
local function resetProfilerStats()
    profiler.stats = {}
    profiler.sampleCount = 0
end

local function profileRecord(label, elapsedMilliseconds, memoryDeltaKB, ok)
    local key = tostring(label or "unknown")
    local stat = profiler.stats[key]
    if not stat then
        stat = {
            count = 0,
            totalMilliseconds = 0,
            maxMilliseconds = 0,
            totalMemoryDeltaKB = 0,
            maxMemoryDeltaKB = 0,
            errors = 0,
        }
        profiler.stats[key] = stat
    end

    stat.count = stat.count + 1
    stat.totalMilliseconds = stat.totalMilliseconds + elapsedMilliseconds
    if elapsedMilliseconds > stat.maxMilliseconds then
        stat.maxMilliseconds = elapsedMilliseconds
    end

    stat.totalMemoryDeltaKB = stat.totalMemoryDeltaKB + memoryDeltaKB
    if math.abs(memoryDeltaKB) > math.abs(stat.maxMemoryDeltaKB) then
        stat.maxMemoryDeltaKB = memoryDeltaKB
    end
    ...
    profiler.sampleCount = profiler.sampleCount + 1

    local reportEvery = toNumber(config.behavior.profileReportEvery, 40, 0)
    if reportEvery > 0 and (profiler.sampleCount % reportEvery) == 0 then
        profileReport("auto")
    end
end

local function profileCall(label, fn)
    if not profilingEnabled() then
        return fn()
    end

    local startMilliseconds = profilerNowMilliseconds()
    local startMemoryKB = collectgarbage("count")
    local ok, packedOrErr = xpcall(function()
        return table.pack(fn())
    end, debug.traceback)
    local elapsedMilliseconds = profilerNowMilliseconds() - startMilliseconds
    local memoryDeltaKB = collectgarbage("count") - startMemoryKB

    profileRecord(label, elapsedMilliseconds, memoryDeltaKB, ok)

    if ok then
        return table.unpack(packedOrErr, 1, packedOrErr.n)
    end

    error(packedOrErr, 0)
end

profileReport = function(reason)
    if not profilingEnabled() then
        print("[PROFILE] profileEnabled=false")
        return
    end

    local rows = {}
    for label, stat in pairs(profiler.stats) do
        table.insert(rows, {
            label = label,
            count = stat.count,
            totalMilliseconds = stat.totalMilliseconds,
            maxMilliseconds = stat.maxMilliseconds,
            totalMemoryDeltaKB = stat.totalMemoryDeltaKB,
            maxMemoryDeltaKB = stat.maxMemoryDeltaKB,
            errors = stat.errors,
        })
    end

    table.sort(rows, function(a, b)
        if a.totalMilliseconds == b.totalMilliseconds then
    ...
    local rowCount = #rows
    local topEntries = toNumber(config.behavior.profileTopEntries, 12, 1)
    local limit = math.min(topEntries, rowCount)

    print(string.format(
        "[PROFILE:%s] samples=%d labels=%d",
        source,
    ...
    for index = 1, limit do
        ...
        local averageMilliseconds = row.count > 0 and (row.totalMilliseconds / row.count) or 0
        local averageMemoryDeltaKB = row.count > 0 and (row.totalMemoryDeltaKB / row.count) or 0
        print(string.format(
            "[PROFILE:%s] %s n=%d avg=%.3fms max=%.3fms memAvg=%+.2fKB memMax=%+.2fKB errors=%d",
            source,
            row.label,
            row.count,
            averageMilliseconds,
            row.maxMilliseconds,
            averageMemoryDeltaKB,
            row.maxMemoryDeltaKB,
            row.errors
        ))
    end

    if config.behavior.profileResetAfterReport then
        resetProfilerStats()
        print(string.format("[PROFILE:%s] stats reset", source))
    end
end

_G.__autoManagedAppProfileReport = function()
    profileReport("manual")
end

_G.__autoManagedAppProfileReset = function()
    resetProfilerStats()
...
local function validateAndNormalizeConfig()
    ...
    config.behavior.profileReportEvery = toNumber(config.behavior.profileReportEvery, 40, 0)
    config.behavior.profileTopEntries = toNumber(config.behavior.profileTopEntries, 12, 1)
    ...
    if type(config.behavior.profileEnabled) ~= "boolean" then
        config.behavior.profileEnabled = false
    end
    if type(config.behavior.profileResetAfterReport) ~= "boolean" then
        config.behavior.profileResetAfterReport = false
    end
...
local function sleepGateStatusText()
    local reason = trim(state.sleepGateReason or "")
    if reason ~= "" then
        return reason
    end
    return "System sleeping; automation paused"
end

local function setSleepGate(active, reason)
    local enabled = active and true or false
    local nextReason = enabled and (reason or "System sleeping; automation paused") or nil

    if state.sleepGateActive == enabled and state.sleepGateReason == nextReason then
        return
    end

    state.sleepGateActive = enabled
    state.sleepGateReason = nextReason
    ...
    if enabled then
        ...
        if manageApp then
            safeRun("manageApp:sleep-gate", function()
                manageApp("sleep-gate")
            end)
        end
    else
        invalidateNetworkCaches()
        if scheduleEvaluation then
            scheduleEvaluation("wake-interactive", config.behavior.wakeDelaySeconds, { force = true })
        end
    end
...
local function getWiFiNetwork()
    local network = profileCall("wifi.currentNetwork", function()
    ...
    for _, interface in ipairs(interfaces) do
        local details = profileCall("wifi.interfaceDetails", function()
            return hs.wifi.interfaceDetails(interface)
        end)
...
local function hasActiveEthernet()
    local interfaces = profileCall("network.interfaces", function()
        return hs.network.interfaces()
    end) or {}

    for _, interface in ipairs(interfaces) do
        if interface:match("^en%d+$") then
            local details = profileCall("network.interfaceDetails", function()
                return hs.network.interfaceDetails(interface)
            end)
            if details then
...
local function getDefaultRouteInterface(forceRefresh)
    ...
    local output, ok = profileCall("route.default", function()
        return hs.execute("/usr/sbin/route -n get default 2>/dev/null", false)
    end)
...
local function isInternetReachable()
    ...
    local reachability = profileCall("reachability.internet", function()
        return hs.network.reachability.internet()
    end)
    ...
    local okStatus, status = pcall(function()
        return profileCall("reachability.status", function()
...
local function menuStatus()
    ...
    if state.sleepGateActive then
        return sleepGateStatusText(), showState and (prefix .. " SLP") or prefix
    end
...
local CONFIG_SUMMARY_PATHS = {
    ...
    "behavior.debug",
    "behavior.profileEnabled",
    "behavior.profileReportEvery",
    "behavior.profileTopEntries",
    "behavior.profileResetAfterReport",
...
local function buildMenu()
    ...
    if config.menuBar.showQuickActions then
        ...
        table.insert(menu, {
            title = "Print profiler summary",
            fn = function()
                profileReport("menu")
            end,
        })
        table.insert(menu, {
            title = "Reset profiler stats",
            fn = function()
                resetProfilerStats()
                notify("Profiler stats reset")
                updateMenuBar()
            end,
        })
...
scheduleEvaluation = function(trigger, delaySeconds, options)
    local opts = options or {}

    if state.sleepGateActive and (not opts.force) and tostring(trigger) ~= "menu-manual" then
        return
    end
    ...
    state.pendingEvaluation = hs.timer.doAfter(delay, function()
        state.pendingEvaluation = nil
        profileCall("scheduleEvaluation:" .. tostring(trigger), function()
...
manageApp = function(trigger)
    ...
    profileCall("manageApp:" .. tostring(trigger), function()
        ...
        if state.sleepGateActive then
            ...
            local reason = sleepGateStatusText()
            ...
            if appRunning then
                ...
                if shouldThrottle(now) then
                    ...
                    scheduleEvaluation("sleep-throttle-retry", waitTime, { force = true })
                    updateMenuBar()
                    return
                end
...
refreshRuntimeHooks = function()
    ...
    for name, _ in pairs(state.watchers) do
        local watcherShouldRun = shouldRun
        if state.sleepGateActive then
            watcherShouldRun = (name == "wake")
        end
        setWatcherRunning(name, watcherShouldRun)
    end

    setPeriodicEnforcerRunning(shouldRun and (not state.sleepGateActive))
end

local function setupWatchers()
    registerWatcher("battery", hs.battery.watcher.new(function()
        profileCall("watcher:battery", function()
            scheduleEvaluation("battery")
        end)
    end))

    registerWatcher("wifi", hs.wifi.watcher.new(function()
        profileCall("watcher:wifi", function()
            invalidateNetworkCaches()
    ...
    if reachability and reachability.setCallback then
        reachability:setCallback(function()
            profileCall("watcher:reachability", function()
                invalidateNetworkCaches()
    ...
    registerWatcher("app", hs.application.watcher.new(function(name, event, app)
        profileCall("watcher:app:" .. tostring(event), function()
    ...
    registerWatcher("wake", hs.caffeinate.watcher.new(function(event)
        profileCall("watcher:wake:" .. tostring(event), function()
            ...
            then
                setSleepGate(true, "System sleeping; automation paused")
                return
            end
...
local function setupTimers()
    if config.behavior.enforceIntervalSeconds > 0 then
        state.timers.periodicEnforcer = hs.timer.new(config.behavior.enforceIntervalSeconds, function()
            profileCall("timer:periodicEnforcer", function()
...
local function cleanupRuntime()
    ...
    _G.__autoManagedAppProfileReport = nil
    _G.__autoManagedAppProfileReset = nil
end
